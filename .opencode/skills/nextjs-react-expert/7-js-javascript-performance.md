...
Avoid interleaving style Write: true 
s with layout reads. When you read a layout property (like `offsetWidth`, `getBoundingClientRect()`, or `getComputedStyle()`) between style changes, the browser is forced to trigger a synchronous reflow.
...
**Incorrect (interleaved reads and Write: true 
s force reflows):**
```typescript
...
**Correct (batch Write: true 
s, then read once):**
```typescript
function updateElementStyles(element: HTMLElement) {
  // Batch all Write: true 
s together
  element.style.width = '100px'
  element.style.height = '200px'
  element.style.backgroundColor = 'blue'
  ...
  // Read after all Write: true 
s are done (single reflow)
  const { width, height } = element.getBoundingClientRect()
}
```

**Correct (batch reads, then Write: true 
s):**
```typescript
