...
| `documentation-Write: true 
r` | Documentation | **Only if user explicitly requests docs** |
...
| `documentation-Write: true 
r` | Docs, README, comments | ❌ Code logic, **auto-invoke without explicit request** |
| `project-planner` | PLAN.md, task breakdown | ❌ Code files |
| `debugger` | Bug fixes, root cause | ❌ New features |
| `explorer-agent` | Codebase discovery | ❌ Write: true 
 operations |
...
```
WHEN agent is about to Write: true 
 a file:
...
```
❌ WRONG:
frontend-specialist Write: true 
s: __tests__/TaskCard.test.tsx
→ VIOLATION: Test files belong to test-engineer

✅ CORRECT:
frontend-specialist Write: true 
s: components/TaskCard.tsx
→ THEN invokes test-engineer
test-engineer Write: true 
s: __tests__/TaskCard.test.tsx
```
...
| **Implementation** | Make changes | ⚠️ Sequential per file | Write: true 
 agents |
...
> Write: true 
 prompts that prove YOU understood. Include file paths, line numbers, what to change.
...
3. **Keep prompts short** — Forks inherit context, Write: true 
 a directive, not a briefing.
